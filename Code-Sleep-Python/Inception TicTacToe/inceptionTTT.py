        ...
        for i in WinCombo:                                                                      
            if((self.bState[i[0]]+self.bState[i[1]]+self.bState[i[2]])==3): 
                ...
                if not (0 in self.bState):       
                    # if there is no 0 in button state , the game is done
                    self.start_label.config(text="Match drawn!!")                              
                     # label if match is drawn
                    self.GameDone=TRUE
                    for k in range(1,10):
                        self.button_lst[k].configure(state=DISABLED)
                    break
...
# Below lines are no longer necessary
# root = Tk()
# root.title("Tic Tac Toe")
# root.geometry("500x530")
# root.config(bg="green")
# leaving in case it's changed to this implementation type
appl = MainGUI()
appl.mainloop()
